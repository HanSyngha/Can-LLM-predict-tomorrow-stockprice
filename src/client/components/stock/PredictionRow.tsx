...
export function PredictionRow({ prediction, market, isExpanded, onToggle }: PredictionRowProps) {
  ...
  return (
    <tr className="hover:bg-slate-50 dark:hover:bg-[#2c2c2e] transition-colors">
      ...
      <td className={`px-3 sm:px-6 py-4 font-semibold ${directionColor(prediction.direction)}`}>
        {directionLabel(prediction.direction)}
      </td>
      <td className={`px-3 sm:px-6 py-4 font-semibold ${directionColor(prediction.actual_direction)}`}>
      ...
      <td className={`px-3 sm:px-6 py-4 ${changeRateColor(prediction.actual_change_rate)}`}>
        {formatPercent(prediction.actual_change_rate)}
      </td>
      <td className="px-3 sm:px-6 py-4 text-slate-700 dark:text-slate-300">
        {formatPrice(prediction.actual_close_price, market)}
      </td>
      <td className="px-3 sm:px-6 py-4 text-center">
        {prediction.is_correct != null ? (
          <span className={`px-2 py-0.5 rounded-full text-xs font-bold ${correctnessColor(prediction.is_correct)}`}>
